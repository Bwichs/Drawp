...
MyGame = ig.Box2DGame.extend({
	...
	update: function() {
		...
		if(ig.global.gameState == -1){
			...
			ig.global.levelON = 11;
			ig.global.levelToLoad = "LevelComplete";
		}
		...
		if (ig.input.state('click') && ig.global.gameState == 0 && levelPastMenu == true && ig.global.gameState != -1 ) {
   			if(this.numL>0 && !ig.global.levelSelect && (ig.global.mouseX != ig.input.mouse.x || ig.global.mouseY != ig.input.mouse.y)){
	...
	draw: function() {
		...
		if (gameState != -1 && ig.global.levelON != 11) {
				ig.system.context.fillStyle = "rgb("+this.r+","+this.g+",0)";
